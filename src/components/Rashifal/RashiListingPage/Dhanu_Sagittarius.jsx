import React from "react";

const Dhanu_Sagittarius = () => {
  const details = {
    sign: "Dhanu (Sagittarius)",
    dateRange: "November 23 – December 21",
    rulingPlanet: "Jupiter",
    element: "Fire",
    luckyColor: "Yellow, Orange",
    luckyNumber: "3, 9",
    description:
      "Sagittarius individuals are optimistic, adventurous, philosophical, and freedom-loving. They thrive on learning, exploring new ideas, and seeking truth. Ruled by Jupiter, they are naturally lucky, spiritual, and generous.",
  };

  const daily = [
    "A good day for financial decisions.",
    "Someone may appreciate your help today.",
    "Avoid overthinking and trust the process.",
  ];

  return (
    <section className="py-16 bg-amber-50/40">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Header */}
        <h2 className="text-4xl font-extrabold text-amber-900 text-center mb-8">
          Dhanu (Sagittarius)
        </h2>

        {/* Overview Box */}
        <div className="bg-white border border-amber-100 rounded-2xl shadow p-8 mb-10">
          <h3 className="text-2xl font-semibold text-amber-800 mb-4">
            About Sagittarius
          </h3>

          <p className="text-gray-700 mb-4">{details.description}</p>

          <ul className="space-y-2 text-gray-700">
            <li>
              <span className="font-semibold text-amber-900">Date Range:</span>{" "}
              {details.dateRange}
            </li>
            <li>
              <span className="font-semibold text-amber-900">
                Ruling Planet:
              </span>{" "}
              {details.rulingPlanet}
            </li>
            <li>
              <span className="font-semibold text-amber-900">Element:</span>{" "}
              {details.element}
            </li>
            <li>
              <span className="font-semibold text-amber-900">
                Lucky Colors:
              </span>{" "}
              {details.luckyColor}
            </li>
            <li>
              <span className="font-semibold text-amber-900">
                Lucky Numbers:
              </span>{" "}
              {details.luckyNumber}
            </li>
          </ul>
        </div>

        {/* Daily Insights */}
        <div className="bg-white border border-amber-100 rounded-2xl shadow p-8">
          <h3 className="text-2xl font-semibold text-amber-800 mb-4">
            Today’s Insights
          </h3>

          <ul className="list-disc list-inside text-gray-700 space-y-2">
            {daily.map((point, index) => (
              <li key={index}>{point}</li>
            ))}
          </ul>

          <button className="mt-6 px-6 py-3 bg-amber-600 text-white rounded-full hover:bg-amber-700 transition font-medium">
            Read Full Horoscope
          </button>
        </div>
      </div>
    </section>
  );
};

export default Dhanu_Sagittarius;

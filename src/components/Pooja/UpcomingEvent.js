const UpcomingEvent = [
  {
    id: 1,
    date: "2026-01-20T18:00:00",
    day: "Tuesday",
    category: ["विवाह बंध", "नवग्रह शांति"],
    deity: ["lord hanuman , Navgrah"],
    poojaName: "मंगल दोष निवारण पूजा",
    poojaNameEn: "Mangal Dosh Nivaran Puja",
    mantra: "मंगल वैदिक मंत्र जाप संख्या",
    jaapSankhya: 10000,
    laabh: [
      "वैवाहिक समस्या से समाधान",
      "दुर्घटना से बचाव",
      "साहस में वृद्धि",
      "क्रोध पर नियंत्रण"
    ],
    laabhEn: [
      "Solution to marital problems",
      "Protection from accidents",
      "Increase in courage",
      "Control over anger"
    ],
    cost: {
      single: 501,
      twoPersons: 701,
      familyOf4: 1001,
      familyOf6: 1251,
    },
    place: "संकटमोचन हनुमान मंदिर संगम दरागंज प्रयागराज",
    placeEn: "Sankatmochan Hanuman Mandir Sangam Daraganj Prayagraj",
    time: "11:00 AM to 01:00 PM",
    image: "https://temple.yatradham.org/public/Product/puja-rituals/puja-rituals_2nbk7AFt_202310091711160.jpg",
    description: "मंगल दोष निवारण के लिए विशेष पूजा",
    descriptionEn: "Special puja for Mangal Dosh removal"
  },
  {
    id: 2,
    date: "2026-01-21T11:00:00",
    day: "Wednesday",
    category: ["नौकरी एवं व्यापार वृद्धि"],
    deity: ["lord ganesh"],
    poojaName: "गणेश पूजा एवं गणेश अथर्व शीर्ष पाठ",
    poojaNameEn: "Ganesh Pooja and Ganesh Atharv Shirsh Paath",
    mantra: "गणेश अथर्व शीर्ष पाठ संख्या",
    paathSankhya: 51,
    laabh: [
      "विघ्नों का नाश",
      "धन एवं समृद्धि",
      "बुद्धि एवं ज्ञान में वृद्धि",
      "राहु एवं केतु के नकारात्मक प्रभाव से रक्षा"
    ],
    laabhEn: [
      "Removal of obstacles",
      "Wealth and prosperity",
      "Increase in intelligence and knowledge",
      "Protection from negative effects of Rahu and Ketu"
    ],
    cost: {
      single: 501,
      twoPersons: 701,
      familyOf4: 1001,
      familyOf6: 1251,
    },
    place: "सिद्ध पीठ ओमकार गणेश मंदिर प्रयागराज",
    placeEn: "Siddh Peeth Omkar Ganesh Mandir Prayagraj",
    time: "11:00 AM to 01:00 PM",
    image: "https://shaligram.com/product-img/ganapati-atharvashirsha-paath-OPS-14-16-Zoom.jpg",
    description: "कार्य और व्यापार में वृद्धि के लिए गणेश पूजा",
    descriptionEn: "Ganesh puja for career and business growth"
  },
  {
    id: 3,
    date: "2026-01-22T11:00:00",
    day: "Thursday",
    category: ["लक्ष्मी प्राप्ति पूजा", "शत्रु बंध निवारण पूजा"],
    deity: ["lord vishnu"],
    poojaName: "विष्णु षट्‌नाम पाठ",
    poojaNameEn: "Vishnu Shatnam Paath",
    mantra: "विष्णु षट्‌नाम पाठ संख्या",
    paathSankhya: 21,
    laabh: [
      "धन सुख समृद्धि प्राप्ति",
      "सभी संकटों से छुटकारा",
      "बिगड़े काम बनते हैं"
    ],
    laabhEn: [
      "Acquisition of wealth and prosperity",
      "Relief from all troubles",
      "Resolution of difficult situations"
    ],
    cost: {
      single: 501,
      twoPersons: 701,
      familyOf4: 1001,
      familyOf6: 1251,
    },
    place: "वेणी माधव मंदिर दरागंज प्रयागराज",
    placeEn: "Veni Madhav Mandir Daraganj Prayagraj",
    time: "11:00 AM to 01:00 PM",
    image: "https://static.india.com/wp-content/uploads/2024/03/Shri-Vishnu-Shatnama-Stotra.jpg?impolicy=Medium_Widthonly&w=700",
    description: "धन प्राप्ति और शत्रु निवारण के लिए विष्णु पूजा",
    descriptionEn: "Vishnu puja for wealth acquisition and enemy removal"
  },
  {
    id: 4,
    date: "2026-01-23T11:00:00",
    day: "Friday",
    category: ["सरस्वती पूजा"],
    deity: ["devi ji"],
    poojaName: "सरस्वती पूजन एवं चढ़ावा",
    poojaNameEn: "Saraswati Poojan and Chadawa",
    laabh: [
      "ज्ञान एवं एकाग्रता में वृद्धि",
      "सरस्वती विशेष कृपा"
    ],
    laabhEn: [
      "Increase in knowledge and concentration",
      "Special blessings of Saraswati"
    ],
    cost: {
      single: 501,
      twoPersons: 701,
      familyOf4: 1001,
      familyOf6: 1251,
    },
    place: "संगम तट प्रयागराज",
    placeEn: "Sangam Ghat Prayagraj",
    time: "11:00 AM to 01:00 PM",
    image: "https://shaligram.com/product-img/Vasant%20Panchami-OPS-13-4-Zoom.jpg?h=400",
    description: "ज्ञान और विद्या की देवी सरस्वती की पूजा",
    descriptionEn: "Worship of Goddess Saraswati, the deity of knowledge"
  },
  {
    id: 5,
    date: "2026-01-24T11:00:00",
    day: "Saturday",
    category: ["नवग्रह शांति"],
    deity: ["Navgrah"],
    poojaName: "शनि मूल मंत्र जाप",
    poojaNameEn: "Shani Mool Mantra Jaap",
    mantra: "शनि मूल मंत्र जाप संख्या",
    jaapSankhya: 23000,
    laabh: [
      "शनि दोष से मुक्ति",
      "साढ़े साती में रक्षा",
      "न्याय और सम्मान"
    ],
    laabhEn: [
      "Relief from Shani Dosh",
      "Protection during Sade Sati",
      "Justice and respect"
    ],
    cost: {
      single: 501,
      twoPersons: 701,
      familyOf4: 1001,
      familyOf6: 1251,
    },
    place: "शनि मंदिर प्रयागराज",
    placeEn: "Shani Mandir Prayagraj",
    time: "11:00 AM to 01:00 PM",
    image: "https://mysticalbee.com/wp-content/uploads/2023/01/Shani-Beej-Mantra.jpg",
    description: "शनि दोष निवारण के लिए विशेष पूजा",
    descriptionEn: "Special puja for Shani Dosh removal"
  },
  {
    id: 6,
    date: "2026-01-25T11:00:00",
    day: "Sunday",
    category: ["नवग्रह शांति"],
    deity: ["Surya Dev"],
    poojaName: "सूर्य मंत्र जाप",
    poojaNameEn: "Surya Mantra Jaap",
    mantra: "सूर्य मूल मंत्र जाप संख्या",
    jaapSankhya: 7000,
    laabh: [
      "सूर्य दोष से मुक्ति",
      "स्वास्थ्य में सुधार",
      "सफलता और प्रसिद्धि"
    ],
    laabhEn: [
      "Relief from Surya Dosh",
      "Improvement in health",
      "Success and fame"
    ],
    cost: {
      single: 501,
      twoPersons: 701,
      familyOf4: 1001,
      familyOf6: 1251,
    },
    place: "सूर्य मंदिर प्रयागराज",
    placeEn: "Surya Mandir Prayagraj",
    time: "06:00 AM to 08:00 AM",
    image: "https://images.news18.com/ibnkhabar/uploads/2023/05/%E0%A4%97%E0%A4%BE%E0%A4%AF%E0%A4%A4%E0%A5%8D%E0%A4%B0%E0%A5%80-%E0%A4%AE%E0%A4%82%E0%A4%A4%E0%A5%8D%E0%A4%B0-%E0%A4%95%E0%A5%87-%E0%A4%AB%E0%A4%BE%E0%A4%AF%E0%A4%A6%E0%A5%87-58.jpg?im=Resize,width=807,aspect=fit,type=normal",
    description: "सूर्य दोष निवारण के लिए विशेष पूजा",
    descriptionEn: "Special puja for Surya Dosh removal"
  },
  {
    id: 7,
    date: "2026-01-26T11:00:00",
    day: "Monday",
    category: ["धन वृद्धि", "सुरक्षा"],
    deity: ["durga"],
    poojaName: "ललिता शत्रनाम",
    poojaNameEn: "Lalita Shatrnam",
    paathSankhya: 21,
    laabh: [
      "धन वैभव यश में वृद्धि",
      "स्वास्थ्य और सुरक्षा"
    ],
    laabhEn: [
      "Increase in wealth, prosperity, and fame",
      "Health and protection"
    ],
    cost: {
      single: 501,
      twoPersons: 701,
      familyOf4: 1001,
      familyOf6: 1251
    },
    place: "ललिता देवी मंदिर प्रयागराज",
    placeEn: "Lalita Devi Mandir Prayagraj",
    time: "11:00 AM to 01:00 PM",
    image: "https://d2vlk40wphb5v6.cloudfront.net/public/cms-images/blogs/1171419503_Lalitha-Sahasranam-to-wards-off-black-magic.jpg", // You can replace with the correct image URL
    description: "ललिता शत्रनाम पूजा धन और सुरक्षा के लिए",
    descriptionEn: "Lalita Shatrnam puja for wealth and protection"
  }
  ,// ID 8: 27th January (Tuesday)
  {
    id: 8,
    date: "2026-01-27T11:00:00",
    day: "Tuesday",
    category: ["नवग्रह शांति", "पितृ दोष निवारण"],
    deity: ["Navgrah"],
    poojaName: "केतु जाप",
    poojaNameEn: "Ketu Jaap",
    jaapSankhya: 17000,
    laabh: [
      "केतु दोष निवारण",
      "तनाव से मुक्ति",
      "करियर में सफलता"
    ],
    laabhEn: [
      "Ketu Dosh removal",
      "Relief from stress",
      "Success in career"
    ],
    cost: {
      single: 501,
      twoPersons: 701,
      familyOf4: 1001,
      familyOf6: 1251
    },
    place: "संकट मोचन हनुमान मंदिर प्रयागराज",
    placeEn: "Sankat Mochan Hanuman Mandir Prayagraj",
    time: "11:00 AM to 01:00 PM",
    image: "https://www.astrokaka.in/uploads/products/2021/04/ketu-grah-shanti-puja-pauranik-vidhi.jpg", // Replace with the correct image URL
    description: "केतु दोष निवारण के लिए विशेष पूजा",
    descriptionEn: "Special puja for Ketu Dosh removal"
  },

  // ID 9: 28th January (Wednesday)
  {
    id: 9,
    date: "2026-01-28T11:00:00",
    day: "Wednesday",
    category: ["नवग्रह शांति", "व्यापार एवं करियर"],
    deity: ["Navgrah"],
    poojaName: "बुद्ध वेदिक मंत्र जाप",
    poojaNameEn: "Buddh Vedic Mantra Jaap",
    jaapSankhya: 9000,
    laabh: [
      "व्यापार एवं करियर में सफलता",
      "आत्मविश्वास",
      "बुद्धि एवं ज्ञान में वृद्धि"
    ],
    laabhEn: [
      "Success in business and career",
      "Self-confidence",
      "Increase in intelligence and knowledge"
    ],
    cost: {
      single: 501,
      twoPersons: 701,
      familyOf4: 1001,
      familyOf6: 1251
    },
    place: "सिद्ध पीठ ओमकार गणेश मंदिर दरागंज प्रयागराज",
    placeEn: "Siddh Peeth Omkar Ganesh Mandir Daraganj Prayagraj",
    time: "11:00 AM to 01:00 PM",
    image: "https://i0.wp.com/pujadhaam.com/wp-content/uploads/2024/06/bbbbb.jpg?resize=990%2C990&ssl=1", // Replace with the correct image URL
    description: "व्यापार और करियर में सफलता के लिए विशेष पूजा",
    descriptionEn: "Special puja for success in business and career"
  },
  {
    id: 10,
    date: "2026-01-29T11:00:00",
    day: "Thursday",
    category: ["नवग्रह शांति", "स्वास्थ्य समृद्धि"],
    deity: ["Navgrah", "Lord Vishnu"],
    poojaName: "विष्णु पूजन एवं बृहस्पति वेदिक मंत्र जाप (एकादशी विशेष)",
    poojaNameEn: "Vishnu Poojan and Brihaspati Vedic Mantra Jaap (Ekadashi Special)",
    jaapSankhya: 19000,
    laabh: [
      "विवाह और पारिवारिक सुख",
      "भय निवारण",
      "जीवन में स्थिरता"
    ],
    laabhEn: [
      "Marriage and family happiness",
      "Removal of fears",
      "Stability in life"
    ],
    cost: {
      single: 501,
      twoPersons: 701,
      familyOf4: 1001,
      familyOf6: 1251
    },
    place: "वेणी माधव मंदिर प्रयागराज",
    placeEn: "Veni Madhav Mandir Prayagraj",
    time: "11:00 AM to 01:00 PM",
    image: "https://bhaktisandesh.com/wp-content/uploads/2025/04/%E0%A4%97%E0%A5%81%E0%A4%B0%E0%A5%81-%E0%A4%95%E0%A4%BE-%E0%A4%B5%E0%A5%88%E0%A4%A6%E0%A4%BF%E0%A4%95-%E0%A4%AE%E0%A4%82%E0%A4%A4%E0%A5%8D%E0%A4%B0.webp", // Replace with the correct image URL
    description: "विवाह, पारिवारिक सुख और भय निवारण के लिए विष्णु पूजा",
    descriptionEn: "Vishnu puja for marriage, family happiness, and removal of fears"
  }

];

export default UpcomingEvent;